...
module.exports = {
  ...
  async execute(interaction: { reply(arg0: { content?: any; embeds?: EmbedBuilder[]; ephemeral?: boolean }): void; options: { getString: (arg0: string) => string }; locale: string; [key: string]: any }) {
    ...
    if (!activities[0]) {
      const notfoundEmbed = new EmbedBuilder().setAuthor({ name: await getLocale(interaction.locale, 'top-nostat') }).setColor('Red')
      return await interaction.reply({ embeds: [notfoundEmbed], ephemeral: true })
    }
